...
import { getTimePattern, REOCCURRANCE_ATTRIBUTE } from './timeUtil';
import HueBridgeModelError from '../HueBridgeModelError';

const RECURRING_RANDOMIZED_TIMER_REGEX = new RegExp(`^R(?<times>[0-9]{0,2})/PT${getTimePattern()}A${getTimePattern('random')}$`);

type TimeValue = string | RecurringRandomizedTimer;

export default class RecurringRandomizedTimer implements BridgeTime {

  private time: HueTime;

  private random: HueTime;
